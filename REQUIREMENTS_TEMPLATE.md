...
1. Copy the header row and one empty row (replace `_…_` placeholders).
2. Add one row per requirement; keep **Acceptance criteria** as short bullet points.
3. Use **Dependency** to list IDs this requirement depends on (e.g. `VR-002, VR-003`).
4. Update **Status** as work progresses: `To Do` → `In Progress` → `In Review` → `Done` / `Blocked`.
